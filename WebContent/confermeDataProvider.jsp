<%@ page language="java" contentType="text/html; charset=US-ASCII" pageEncoding="US-ASCII"%>
<%@ taglib prefix="f"  uri="http://java.sun.com/jsf/core"%>
<%@ taglib prefix="h"  uri="http://java.sun.com/jsf/html"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=US-ASCII">
<title>Insert title here</title>
</head>
<body style="background-color: LIGHTGOLDENRODYELLOW">
<f:view>
<h:form>
<h1>Overview of your credenzials</h1>
<ul>
<li>${providerController.name}</li>
<li>${providerController.street},${customerController.streetNumber}</li>
<li>${providerController.city}</li>
<li>${providerController.country}</li>
<li>${providerController.state}</li>
<li>${providerController.zipcode}</li>
</ul>
<div>	<h:commandButton value="Conferme"  action="#{providerController.createProvider}" />
	
</div>
</h:form>
</f:view>
</body>
</html>